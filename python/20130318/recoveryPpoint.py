...
def extractTargetUser(amebaIdList):
    ...
    for amebaId in amebaIdList:
        ...
        if exists:
            print amebaId
            db2.CommonUserPpointRecovery.update({'_id' : userId }, {"$set" : {"quest_0315_asparagus_01" : { "actCounts" : [ 1 ], "data" : [ "" ], "recoveryTimes" : [ 1363573416102 ] }}})

        else:
            print amebaId
            db2.CommonUserPpointRecovery.insert({'_id' : userId , "quest_0315_asparagus_01" : { "actCounts" : [ 1 ], "data" : [ "" ], "recoveryTimes" : [ 1363573416102 ]}})
